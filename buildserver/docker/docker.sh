...
function build(){
  ...
  docker build \
    ...
    # Build s2i images
    s2i --loglevel 5 --ref OPENSHIFT_JENKINS_PIPELINE --context-dir /buildserver/s2i/jenkins-ci build https://github.com/Gepardec/Hogarama openshift/jenkins-2-centos7 ${JENKINS_TAG}
